...
class TrustServiceStatusList {
	...
	constructor(el: Element) {
		...
		this.trustServiceProviderList = Array.from(el.querySelectorAll('TrustServiceProvider')).map(
			(el) => new TrustServiceProvider(el)
		);
	}
}
...
class SchemeInformation {
	...
	constructor(el: Element) {
		...
		this.schemeOperatorName = Array.from(
			el.querySelector('SchemeOperatorName')?.querySelectorAll('Name') || []
		).map(toMultiLangString);
		...
		this.schemeInformationURI = Array.from(
			el.querySelector('SchemeInformationURI')?.querySelectorAll('URI') || []
		).map(toMultiLangString);
		this.statusDeterminationApproach =
			el.querySelector('StatusDeterminationApproach')?.textContent || '';
		...
		if (el.querySelector('PointersToOtherTSL')) {
			this.otherTSLPointer = Array.from(el.querySelectorAll('OtherTSLPointer')).map(
				(el) => new OtherTSLPointer(el)
			);
		}
	...
	constructor(el: Element) {
		...
		if (el.querySelector('AdditionalInformation')) {
			this.additionalInformation = new AdditionalInformation(
				el.querySelector('AdditionalInformation')!
			);
		}
		if (el.querySelectorAll('ServiceDigitalIdentity').length > 0) {
			this.serviceDigitalIdentities = Array.from(el.querySelectorAll('ServiceDigitalIdentity')).map(
				(el) => new ServiceDigitalIdentity(el)
			);
		}
	}
}
...
class AdditionalInformation {
	...
	constructor(el: Element) {
		if (el.querySelectorAll('TextualInformation').length > 0) {
			this.textualInformation = Array.from(el.querySelectorAll('TextualInformation')).map(
				toMultiLangString
			);
		}
	}
...
class ServiceDigitalIdentity {
	...
	constructor(el: Element) {
		this.digitalIdentites = Array.from(el.querySelectorAll('DigitalId')).map(
			(el) => new DigitalIdentity(el)
		);
	}
}
...
class TrustServiceProvider {
	...
	constructor(el: Element) {
		...
		this.tspServices = Array.from(el.querySelectorAll('TSPService')).map(
			(el) => new TSPService(el)
		);
	}
}
...
class TSPInformation {
	...
	constructor(el: Element) {
		this.tspName = Array.from(el.querySelector('TSPName')?.querySelectorAll('Name') || []).map(
			toMultiLangString
		);
		this.tspTradeName = Array.from(
			el.querySelector('TSPTradeName')?.querySelectorAll('Name') || []
		).map(toMultiLangString);
		this.tspAddress = new Address(el.querySelector('TSPAddress')!);
		this.tspInformationURI = Array.from(
			el.querySelector('TSPInformationURI')?.querySelectorAll('URI') || []
		).map(toMultiLangURI);
		if (el.querySelectorAll('TSPInformationExtensions').length > 0) {
			this.tspInformationExtensions = Array.from(
				el.querySelectorAll('TSPInformationExtensions')
			).map((el) => new Extension(el));
		}
	}
}
...
class Extension {
	...
	constructor(el: Element) {
		...
		this.extensionOID =
			queryWithNamespace(el, NS_ETSI_02231_V2, 'ExtensionOID')?.textContent || null;
		this.extensionValue =
			queryWithNamespace(el, NS_ETSI_02231_V2, 'ExtensionValue')?.textContent || null;
		if (el.querySelectorAll('AdditionalServiceInformation').length > 0) {
			this.additionalServiceInformation = Array.from(
				el.querySelectorAll('AdditionalServiceInformation')
			).map((el) => new AdditionalServiceInformation(el));
		}
	}
}
...
class ServiceInformation {
	...
	constructor(el: Element) {
		...
		this.serviceDigitalIdentity = new ServiceDigitalIdentity(
			el.querySelector('ServiceDigitalIdentity')!
		);
		...
		if (el.querySelectorAll('SchemeServiceDefinitionURI').length > 0) {
			this.schemeServiceDefinitionURI = Array.from(
				el.querySelectorAll('SchemeServiceDefinitionURI')
			).map(toMultiLangURI);
		}
		if (el.querySelectorAll('ServiceSupplyPoints').length > 0) {
			this.serviceSupplyPoints = Array.from(el.querySelectorAll('ServiceSupplyPoint')).map(
				(el) => new ServiceSupplyPoint(el)
			);
		}
		if (el.querySelectorAll('ServiceInformationExtensions').length > 0) {
			this.serviceInformationExtensions = Array.from(el.querySelectorAll('Extension')).map(
				(el) => new Extension(el)
			);
		}
	}
...
class PolicyOrLegalNotice {
	...
	constructor(el: Element) {
		...
		if (el.querySelectorAll('TSLLegalNotice').length > 0) {
			this.tslLegalNotice = Array.from(el.querySelectorAll('TSLLegalNotice')).map(
				toMultiLangString
			);
		}
	}
...
class Address {
	...
	constructor(el: Element) {
		this.postalAddresses = Array.from(el.querySelectorAll('PostalAddress')).map(
			(el) => new PostalAddress(el)
		);
		this.electronicAddress = Array.from(
			el.querySelector('ElectronicAddress')?.querySelectorAll('URI') || []
		).map(toMultiLangURI);
	}
}
