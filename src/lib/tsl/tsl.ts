...
export function getMultilangText(multilang: MultiLangString[], lang: string | undefined = undefined): string {
...
export function getMultilangURI(multilang: MultiLangURI[], lang: string | undefined = undefined): string {
    if (multilang.length === 0) {
        return ''
    }
    if (!lang) {
...
export function getPrimaryLocation(schemeInfo: ISchemeInformation) {
    const loc = schemeInfo.otherTSLPointer?.find((pointer) => pointer.additionalInformation?.textualInformation?.find((text) => text.string === oid_tsl_p_loc))
    return loc ? loc.tslLocation : '-'
}

export function getBackupLocation(schemeInfo: ISchemeInformation) {
    const loc = schemeInfo.otherTSLPointer?.find((pointer) => pointer.additionalInformation?.textualInformation?.find((text) => text.string === oid_tsl_b_loc))
    return loc ? loc.tslLocation : '-'
}
